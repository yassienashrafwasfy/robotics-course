...
# Modified by yassienashrafwasfy
#
...
class DRLGazebo(Node):
    def __init__(self):
        ...
        ************************************************************"""
        ...
        # Initialize entity directory path
        ...
        ************************************************************"""
        ...
        self.goal_pose_pub = self.create_publisher(Pose, 'goal_pose', QoSProfile(depth=1, durability=DurabilityPolicy.TRANSIENT_LOCAL)) # QoSProfile(depth=10))

        # Initialise client
        self.delete_entity_client       = self.create_client(DeleteEntity, '/world/default/remove')
        self.spawn_entity_client        = self.create_client(SpawnEntity, '/world/default/create')
        self.reset_simulation_client    = self.create_client(Empty, '/world/default/control')
        # self.gazebo_pause disabled for Gazebo Harmonic
    ...
    *******************************************************************************"""
    ...
    def reset_simulation(self):
        ...
        subprocess.Popen([
            ...
            '--timeout', '1000',
            '--req', 'name: "waffle_pi", position: {x: 0.0, y: 0.0, z: 0.01}, orientation: {w: 1.0}'
        ])
    ...
    def spawn_entity(self):
        ...
        req.entity_factory.name = self.entity_name
        req.entity_factory.sdf = self.entity
        req.entity_factory.pose = goal_pose
        time.sleep(0.5)
        self.spawn_entity_client.call_async(req)

    def get_obstacle_coordinates(self):
        tree = ET.parse(os.path.join(self.entity_dir_path, 'inner_walls', 'model.sdf'))
        root = tree.getroot()
