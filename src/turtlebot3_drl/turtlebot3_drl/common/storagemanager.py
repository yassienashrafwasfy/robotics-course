...
class StorageManager:
    def __init__(self, name, load_session, load_episode, device, stage):
        ...
        # Search for the 'turtlebot3_drl' directory
        turtlebot3_drl_path = glob.glob(os.path.join(current_dir, 'src', '**', 'turtlebot3_drl'), recursive=True)
        if not turtlebot3_drl_path:
            raise FileNotFoundError("Could not find the 'turtlebot3_drl' package in the src folder.")

        self.machine_dir = os.path.join(turtlebot3_drl_path[0], f'model{socket.gethostname()}')
        if 'trained' in load_session:
            self.machine_dir = os.path.join(turtlebot3_drl_path[0])
