...
class DrlAgent(Node):
    def __init__(self, training, algorithm, load_session="", load_episode=0, real_robot=0):
        ...
        if (not self.training and not self.load_session):
            quit("\033[1m" + "\033[93m" + "Invalid: but no model to load specified, see readme for correct format" + "\033[0m}",self.training)
        self.device = util.check_gpu()
        self.sim_speed = 1 #util.get_simulation_speed(util.stage) if not self.real_robot else 1
        ...
        if not self.real_robot: pass  # Gazebo Harmonic - no pause needed
            # self.gazebo_pause disabled for Gazebo Harmonic
            # self.gazebo_unpause disabled for Gazebo Harmonic
        self.process()


    def process(self):
        pass  # pause disabled for Gazebo Harmonic
        while (True):
            ...
            pass  # unpause disabled for Gazebo Harmonic
            time.sleep(0.5)
